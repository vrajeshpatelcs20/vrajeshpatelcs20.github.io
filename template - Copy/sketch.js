  ...
  this.checkIfHit = function (point) {
    for (let i = 0; i < this.numOfShips; i++) {
      if (this.ships[i].checkLocation(point)) {
        ...
        if (this.ships[i].hitPoints == 0) {
          return this.removeShip(i);
        }
        else {
          return this.shipHit(this.ships[i].name);
        }
      }
    }
    return false;
    ...
    $(square)
      ...
      .on("click", function () {
        if (!$(this).hasClass("used")) {
          ...
          let bool = cpuFleet.checkIfHit(num);
          if (false == bool) {
...
var bot = {
  ...
  getRandomMoves: function (possibleMoves) {
    while (possibleMoves.length != 0) {
      ...
      if (possibleMoves[dir] == "up") {
        if (
          bot.randPool.some(function (x) {
            return x == bot.current - 10;
          })
        ) {
  ...
  getNumericalDirection: function (dir) {
    ...
    if (
      bot.attempted.some(function (x) {
        return x == bot.current;
      }) &&
      bot.specialHits.length == 0
    ) {
      bot.current = bottomBoard.currentHits[0];
      if (bot.back_count > 1) {
        bot.special = true;
  ...
  specialCase: function () {
    ...
    if (bot.case1) {
      ...
      if (bot.specialHits.length == 0) {
        for (let i = 0; i < bottomBoard.currentHits.length; i++) {
  ...
  getNewCurrent: function (direction) {
    let difference = bottomBoard.currentHits.length - bot.sizeOfShipSunk;
    if (bot.direction == "up") {
      bot.direction = "down";
      if (difference > 1) {
...
$(document).ready(function () {
  for (let i = 1; i <= 100; i++) {
    ...
    if (i < 11) {
      ...
      $(".bottom").prepend(
        "<span class='aTops'>" + Math.abs(i - 11) + "</span>"
      );
      $(".grid").append(
        "<li class='points offset1 " + i + "'><span class='hole'></span></li>"
      );
    }
    else {
      $(".grid").append(
        "<li class='points offset2 " + i + "'><span class='hole'></span></li>"
      );
    }
    if (i == 11) {
      $(".top").prepend(
        "<span class='aTops hidezero'>" + Math.abs(i - 11) + "</span>"
      );
      $(".bottom").prepend(
        "<span class='aTops hidezero'>" + Math.abs(i - 11) + "</span>"
      );
    ...
    if (i > 90) {
      $(".top").append(
        "<span class='aLeft'>" +
        String.fromCharCode(97 + (i - 91)).toUpperCase() +
        "</span>"
      );
      $(".bottom").append(
        "<span class='aLeft'>" +
...
function randomSetup(fleet) {
  ...
  if (fleet.currentShip >= fleet.numOfShips) {
    return;
  } // regard against undefined length
  ...
  if (orien < 6) {
    ...
    if (fleet == cpuFleet) {
      checkOverlap(randNum, length, "horz", fleet);
    }
    else {
    ...
    if (fleet == cpuFleet) {
      checkOverlap(randNum, length, "vert", fleet);
    }
    else {
...
function setShip(location, ship, orientation, genericFleet, type) {
  if (!checkOverlap(location, ship.length, orientation, genericFleet)) {
    if (orientation == "horz") {
      ...
      $(".text").text(
        output.placed(
          genericFleet.ships[genericFleet.currentShip].name + " has"
        )
      );
      for (var i = location; i < location + ship.length; i++) {
        $(".bottom ." + i).addClass(
          genericFleet.ships[genericFleet.currentShip].name
        );
        $(".bottom ." + i)
          .children()
          .removeClass("hole");
